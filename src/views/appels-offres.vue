      ...
      <div class="tab-content p-3 md:p-16" v-if="current_tab == 'resultats'">
        <div v-if="appels_offres.data.length > 0">
          <div
            ...
            v-for="(document, index) in appels_offres.data"
            :key="index"
          >
            ...
            <p
              class="text-base leading-6 text-subtitlegray tracking-tight my-2"
  ...
  methods: {
    async searchSomething() {
      if (this.search_value.trim() == "") {
        ...
        this.loader = true;
        try {
          await services.search_appels(this.search_value).then((resp) => {
            this.loader = false;
