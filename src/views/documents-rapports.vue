  ...
  <main class="p-3 sm:p-16">
    <div class="px-3 md:px-16">
      ...
      <div v-else-if="loader == false">
        <div class="document-box pt-16" v-for="(el, index) in files_datas.data" :key="index">
          <div class="text-base text-primary">Publié le {{ formatDate(el.created_at, 'LLL') }}</div>
          ...
          <div class="text-base mb-2">
            ...
            <span> {{ el.nbre_downloads }} {{ el.nbre_downloads > 0 ? 'Téléchargements' : 'Téléchargement' }}</span>
          </div>
          <div>
            ...
            <div class="attachment-box w-full flex items-center flex-wrap justify-between">
              ...
              <div class="text-sm leading-5 font-medium">
                <a :href="el.file_url" :download="el.file_name" target="_blank" class="text-primary">Télécharger</a>
              </div>
            </div>
          </div>
          <div class="py-20">
...
export default {
  ...
  methods: {
    ...
    async load_files(current_page) {
      this.loader = true
      await services.get_document_rapports(1).then((response) => {
        console.log('....ici')
        this.loader = false
